...
> 举个栗子，当我们扔硬币时，出现的可能性有两个，要么正面朝上（事件A），要么反面朝上(事件B)，假设出现某事件的可能性只与硬币的 `质量分布θ` 有关系，假设我们做了n组实验（A出现m次），这样能得到两种情况的概率 P(A), P(B),我们要推测出 `质量分布θ` 是多少，也就是 `参数估计` ,即 `质量分布θ` 是多少时，才最可能出现当前实验的结果。抛硬币的事件服从二项分布，那么给定了一组实验的情况下，似然函数是：
> $$
...
$$
\mathrm{P}(\mathrm{Y}=1 | \mathrm{X})=\emptyset(x) 
$$

$$
\mathrm{P}(\mathrm{Y}=0 | \mathrm{X})=1-\emptyset(x)
$$
...
$$
\prod_{i=1}^{N}\left[\emptyset\left(x_{i}\right)\right]^{y_{i}} \cdot\left[1-\emptyset\left(x_{i}\right)\right]^{1-y_{i}}
$$
